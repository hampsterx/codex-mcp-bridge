...
import { executeReview, scaleTimeoutForDepth } from "../../src/tools/review.js";

const origMcpEnv = process.env["CODEX_MCP_SERVERS"];
const origCodexHome = process.env["CODEX_HOME"];

describe("scaleTimeoutForDepth (deep)", () => {
  it("returns base timeout for 0 files", () => {
    expect(scaleTimeoutForDepth("deep", { files: 0, insertions: 0, deletions: 0 })).toBe(180_000);
  });

  it("scales linearly with file count", () => {
    expect(scaleTimeoutForDepth("deep", { files: 1, insertions: 10, deletions: 5 })).toBe(210_000);
    expect(scaleTimeoutForDepth("deep", { files: 5, insertions: 50, deletions: 20 })).toBe(330_000);
    expect(scaleTimeoutForDepth("deep", { files: 10, insertions: 100, deletions: 50 })).toBe(480_000);
  });

  it("caps at HARD_TIMEOUT_CAP (1800s)", () => {
    // 180s + 30s * 60 = 1980s, should cap at 1800s
    expect(scaleTimeoutForDepth("deep", { files: 60, insertions: 2000, deletions: 500 })).toBe(1_800_000);
    expect(scaleTimeoutForDepth("deep", { files: 100, insertions: 5000, deletions: 2000 })).toBe(1_800_000);
  });

  it("scales correctly: 3 files -> 270s, cap reached at 54 files", () => {
    // 3 files: 180 + 90 = 270s
    expect(scaleTimeoutForDepth("deep", { files: 3, insertions: 30, deletions: 10 })).toBe(270_000);
    // 54 files: 180 + 1620 = 1800s (exactly at cap)
    expect(scaleTimeoutForDepth("deep", { files: 54, insertions: 500, deletions: 200 })).toBe(1_800_000);
  });
});

describe("scaleTimeoutForDepth (focused + scan)", () => {
  it("focused: base + per-file, capped at 300s", () => {
    expect(scaleTimeoutForDepth("focused", { files: 0, insertions: 0, deletions: 0 })).toBe(120_000);
    expect(scaleTimeoutForDepth("focused", { files: 3, insertions: 30, deletions: 10 })).toBe(165_000);
    // 120 + 15 * 12 = 300s (exactly at cap)
    expect(scaleTimeoutForDepth("focused", { files: 12, insertions: 200, deletions: 50 })).toBe(300_000);
    // 120 + 15 * 100 = 1620s, capped at 300s
    expect(scaleTimeoutForDepth("focused", { files: 100, insertions: 5000, deletions: 2000 })).toBe(300_000);
  });

  it("scan: constant 120s regardless of file count", () => {
    expect(scaleTimeoutForDepth("scan", { files: 0, insertions: 0, deletions: 0 })).toBe(120_000);
    expect(scaleTimeoutForDepth("scan", { files: 1, insertions: 10, deletions: 5 })).toBe(120_000);
    expect(scaleTimeoutForDepth("scan", { files: 100, insertions: 5000, deletions: 2000 })).toBe(120_000);
  });
});
...
describe("review timeout selection", () => {
  ...
  it("deep mode falls back to static timeout when diffStat unavailable", async () => {
    getUncommittedDiffMock.mockReturnValue("diff --git a/x b/x");
    getDiffStatMock.mockReturnValue(undefined);
    spawnCodexMock.mockResolvedValue({
      stdout: "review", stderr: "", exitCode: 0, timedOut: false,
    ...
    // Deep fallback is 600s (up from legacy 300s — see PLAN decision log).
    expect(result.appliedTimeout).toBe(600_000);
    expect(result.timeoutScaled).toBe(false);
  ...
  it("timeout message includes diff context when timed out with diffStat", async () => {
    ...
    expect(result.response).toContain(`depth: "scan"`);
  });
  ...
  it("scan-mode timeout message does not suggest depth: scan", async () => {
    getUncommittedDiffMock.mockReturnValue("diff --git a/x b/x");
    getDiffStatMock.mockReturnValue({ files: 3, insertions: 30, deletions: 10 });
    spawnCodexMock.mockResolvedValue({
      stdout: "", stderr: "", exitCode: null, timedOut: true,
    ...
    expect(result.response).not.toContain(`depth: "scan"`);
  });
